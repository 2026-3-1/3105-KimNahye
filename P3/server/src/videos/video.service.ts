...
import {
  IWatchLogRepository,
  WATCH_LOG_REPOSITORY,
} from './interfaces/watch-log-repository.interface';
...
export class VideoService {
  ...
  async getVideoDetail(
    ...
    const isOwner = user.role === UserRole.TEACHER && course.teacher.id === user.id;
  ...
  async updateProgress(
    ...
    return {
      videoId,
      watchedDuration: log.watchedDuration,
  ...
  async getProgress(
    ...
    const log = await this.watchLogRepository.findByUserAndVideo(user, video);

    return {
      videoId,
      watchedDuration: log?.watchedDuration ?? 0,
      isCompleted: log?.isCompleted ?? false,
      lastWatchedAt: log?.lastWatchedAt ?? null,
    };
  }
}
